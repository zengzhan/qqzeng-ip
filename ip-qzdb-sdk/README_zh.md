...
2. **内存考虑**：在 C、Go、Java、.NET 中数据库通过内存映射（`mmap`）加载，可在多进程间共享物理内存；Rust 出于 100% Safe Rust 的工程取舍采用全量堆加载（非 mmap），Node.js 同样为全量 Buffer 读入。在 JVM 等托管运行环境中，请确保堆内存上限（Heap limits）能够容纳数据库大小。
