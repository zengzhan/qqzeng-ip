...
所有语言 SDK 均提供一致的接口设计。核心类统一为 `QzdbReader`（C 语言为 `qzdb_reader_t`）：创建即持有实例，按需复用（跨文件/跨版本各自持有实例，或用 `QzdbRegistry` 管理），多实例并发互不干扰。

### 🐍 Python
```python
from qzdb import QzdbReader

# 加载并查询（按路径复用请用 QzdbRegistry）
...
1. **复用实例而非每次重建**：加载数据库涉及解析头部元数据、CRC 校验、预装载字符串索引池，有一定初始化开销。请在程序启动时创建 `QzdbReader` 实例**一次**并长期持有复用（跨文件/跨版本各自持有一个实例，或用 `QzdbRegistry` 管理）。
2. **内存考虑**：在 C、Go、Java、.NET、Rust 中数据库通过内存映射（`mmap`）加载，可在多进程间共享物理内存（Rust 通过 `memmap2` 实现，unsafe 仅隔离在 `map_file()` 一处，其余代码保持安全）；Node.js 为全量 Buffer 读入（非 mmap）。在 JVM 等托管运行环境中，请确保堆内存上限（Heap limits）能够容纳数据库大小。
