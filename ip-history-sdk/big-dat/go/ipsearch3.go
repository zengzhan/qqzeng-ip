...
func GetInstance() *IpSearch {
	once.Do(func() {
		instance = &IpSearch{}
		var err error
		instance, err = LoadDat("./qqzeng-ip-big-3.0.dat")
		if err != nil {
...
func LoadDat(file string) (*IpSearch, error) {
	...
	for i := 0; i < RecordSize; i++ {
		...
		offset := ReadLittleEndian32(data[4+j], data[5+j], data[6+j], data[7+j])
...
func (p *IpSearch) Get(ip string) string {
	...
	y, _ := strconv.Atoi(ips[1])
	prefix := uint32(x)*256+uint32(y)
	intIP := ip2Long(ip)
