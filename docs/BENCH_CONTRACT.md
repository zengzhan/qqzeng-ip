...
- **P1 原生 mmap 加载** → 用 `ult_global`（122 MB）测加载峰值与首查延迟；C# 与 Rust 文件路径入口均使用只读映射，内存字节入口仍按各自 API 的拷贝语义测量。
...
## 12. v1.1 修订(2026-09-06)

### 12.1 各语言分布主循环输入形态钉死(消除跨口径混报)

分布与并发主循环的查询入口必须为**整型 API**(v4 传 uint32、v6 传 128 位/双 u64/16 字节),
字符串解析只允许出现在 §string round-trip 段。各语言现状与整改:

| 语言 | 入口 | 状态 |
|------|------|------|
| C | `qzdb_find_uint` / `qzdb_find_v6` | ✓ 合规 |
| Go | `FindUint` / `FindV6` | ✓ 合规 |
| Rust | `find_uint` / `find_v6` | ✓ 合规 |
| Python | `find_uint` / `find_v6_uint` | ✓ 合规 |
| Node.js | `findUint` / `findBytes`(v4 Number 直传,v6 预分配 Buffer) | ✓ 合规(v1.1 修正) |
| Java | `findUint` / `findV6` | ✓ 合规(v1.1 新增基准) |
| C# | `FindUint` / `Find(ulong,ulong)` | ✓ 合规 |
| PHP | `findUint(gmp_intval($hi))` | ⚠️ 整型入口 ✓,但流推进用 GMP(每次查询含基准自身开销);v1.2 目标:改用原生 int 流(GMP 仅用于 ≥2^63 段) |

**解读规则**:B 口径(热点)数字引用时必须对照本表——PHP 的 B 列数字含 GMP 开销,
系统性低于其 SDK 真实整型能力(直测探针 5.67M vs 契约 319K),两数字不得混用。

### 12.2 多次运行取中位数(降噪要求)

对外引用的单个数字(README/PERFORMANCE.md)**必须为同一状态 ≥3 次完整运行的中位数**,
并记录样本方差;单次运行数字仅限开发过程中间态。本机实测同配置波动可达 ±10%
(极端 ±40%),中位数是最低降噪手段。

### 12.3 新增强制指标:加载耗时与进程内存

吞吐之外,报告 JSON 必须新增两个字段(写于 `environment` 平级):

- `load_time_ms`:从 `open()` 调用到首次可查询的耗时(含 CRC 校验配置下的完整加载),
  3 次中位数。首次查询延迟与重启恢复能力直接相关;
- `rss_mb`:加载完成 + 10 万次查询后的进程常驻内存(各语言取其惯用探针:
  C ru_maxrss / Go runtime.ReadMemStats.Sys / Rust /proc 或 /usr/bin/time /
  Java Runtime.totalMemory-freeMemory / .NET GC.GetTotalMemory / Node
  process.memoryUsage.rss / PHP memory_get_peak_usage / Python resource)。
  大库(122MB)mmap 的常驻内存是选型关键维度,当前契约缺失。

### 12.4 投影数据源(§3.5 第 5 条)列为待决项

API_CONTRACT §3.5 第 5 条规定投影"优先从解码缓存切片(骑缓存)"。Java/PHP/Node/C#
已骑缓存;Go/Rust/Python 为直解(每次只解码请求字段)。两种策略的取舍:
骑缓存利于重复投影,直解利于一次性冷投影。**在 `findFields` 进入契约基准
(新增投影分布)并取得实测数据前,本契约不强制 Go/Rust/Python 改造**;
实测若骑缓存对冷投影显著更差,应修订 §3.5 条文而非强行对齐。

### 12.5 Java 基准纳入合规矩阵

`BenchContract.java`(v1.1 随附)通过 FNV-1a 指纹对拍 12/12 流,合规矩阵从 7/8 → **8/8**。
