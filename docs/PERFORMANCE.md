...
| 口径 | 输入 | 数据 | 场景含义 | 数据来源 |
|------|------|------|----------|----------|
| **A. 整型随机(缓存最不利)** | 整型 API(无 IP 字符串解析) | 50 万随机散布 IP,省级大陆库(8.6MB) | 容量规划用的**保守承诺值** | contract `random` 分布;Java/PHP/Node 为对等直测探针 |
...
| 语言 | A. 整型随机(最不利) | B. 热点缓存 | C. 字符串 find_str | 数据来源 |
|------|-------------------:|-----------:|-------------------:|----------|
| C        | 96.5M | 99.7M  | 10.8M | contract |
| Node.js  | 99.4M | —¹     | 5.5M  | A/C 直测与门禁;B 为字符串口径不混用 |
| Go       | 74.7M | 75.9M  | 9.2M  | contract |
| Java     | 74.9M | —¹     | 11.9M | A/C 直测与门禁;契约基准待建 |
| Rust     | 43.3M | 46.1M  | 8.4M  | contract |
| C#       | 33.1M | 84.6M  | 11.0M | contract |
| PHP      | 5.67M | —¹     | 429K  | A/C 直测与门禁;B 为字符串口径不混用 |
| Python   | 0.97M | 1.08M  | 273K  | contract |
...
(C 96.5M→13.7M、Go 74.7M→23.9M)——内存型数据库的缓存亲和物理规律,宣传时不得只引用小库数字。
