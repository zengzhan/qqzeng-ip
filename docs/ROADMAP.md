...
- **验证结果（2026-08-23）**：Python `test_tlv_meta.py` 6/6（合成库正/反两路）、`test_regression.py` 6/6 + `demo_sample_check.py` 1440/1440 + `test_hostile_vectors.py` OK（真实旧文件回落零变化）；Node `test.js` / `regression_test.js` / `test_suite.js` 全绿（Tier1 379、Tier2 黄金校验 4102/4102）；PHP tier1 新增 TLV 权威断言；C 新增 `tlv_meta_test.c`（真实库注入 TLV，ASan 下 3 用例全绿：TLV 权威/旧文件回落/重复条目 last-wins，兼作 scope 所有权 UAF 回归守卫），run_all_tests.sh 已接线 C-TlvMeta 门禁。八语言终验全部完成（2026-08-23，VM 内临时安装工具链）：Go 1.23 全套件绿（TLV 正/反两测 PASS；TestHostileVectors 直跑编译产物 PASS，峰值 RSS 3456MB）、Rust 1.98 集成 27 + 单元 4 全绿、Java 21 QzdbReaderTest 48/48 + FAILCLOSED_OK（并暴露并修复 `%.0f` 违反 FORMAT §10.5 精确展开契约，改 `BigDecimal(v).toPlainString()`）、C# .NET 10 Tier1 117/117 + Tier2 全 10 库 28,602,081 节点 0 错误（harness 改流式读 CSV 后 exit 0）、PHP 8.0.30 `test.php` PASS + `tier1_test.php` 122/122（夹具补 poolIdxSize@13、PHP 8.0 反射 setAccessible）+ `tier2_hostile.php` HOSTILE_VECTORS_OK。L2 `cross_lang_verify.py` 8 语言 × 61 IP = 427/427 管道输出全同（runner 在 VM 内按 build_all.sh 配方重建；同步修复 batch_query.go 旧模块路径漂移与脚本 JAVA_HOME/C 编译器可移植性）。
- **遗留验证**：无。8 语言单语言终验 + L2 跨语言一致性均已闭环；宿主机 `./run_all.sh` 仍可随时复跑。
...
- **做法**：FORMAT §10.5 落统一契约（整数值→精确十进制展开；非整数→固定 6 位小数；NaN/Inf→""；cast 前必须 \|v\| < 2^63 范围保护，超范围走无小数点定点格式化）；API_CONTRACT §三.2 同步。修复 C#（F0 分支 + float 重载委托 double）、Go（'f',0 分支）、Java（抽包级静态 formatNativeFloat；终验发现 `%.0f` 实为最短 round-trip 补零、违反精确展开，已改 `BigDecimal(v).toPlainString()` 并补 E300 向量锁定）、C（guard 放宽至 ±2^63 + %.0f 分支）、Rust（阈值提至 2^63 + {:.0} 分支）、Node（≥2^53 经 IEEE754 位解码转 BigInt 定点）、PHP（边界 <= 收紧为 <，恰为 2^63 走 %.0F）。
